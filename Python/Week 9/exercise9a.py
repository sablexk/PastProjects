# -*- coding: utf-8 -*-
"""
Created on Wed May 27 09:45:03 2020
...
def doBatchIteration(eta, trainingDocs):
# uses and modifies global vars theta[], w[], and b
# takes in parameter eta
    global theta
    global w, b
...
rand = randint(0, len(posTrainStrings))
randPosTrain = ['', '']
copyPos = posTrainStrings.copy()
for i in range(20):
    randPosTrain.append(copyPos[rand])
    copyPos.remove(copyPos[rand])
    rand = randint(0, len(copyPos))
    
    
randNegTrain = ['','']
rand = randint(0, len(negTrainStrings))
copyNeg = negTrainStrings.copy()
for i in range(20):
    randNegTrain.append(copyNeg[rand])
    copyNeg.remove(copyNeg[rand])
    rand = randint(0, len(copyNeg))
...
posFeatureVectors = []
for s in randPosTrain: #posTrainStrings
    fv = createFeatureVector(s)
    posFeatureVectors.append(fv)

negFeatureVectors = []
for s in randNegTrain:  #negTrainStrings
    fv = createFeatureVector(s)
...
# START NEW CODE
posTestStrings = set()
fpp = open("posTest.txt",mode='r',encoding="ISO-8859-1")
for line in fpp:
    aStr = line.replace('\n','')
    posTestStrings.add(aStr)


negTestStrings = set()
fpp = open("negTest.txt",mode='r',encoding="ISO-8859-1")
for line in fpp:
    aStr = line.replace('\n','')
    negTestStrings.add(aStr)
# END NEW CODE
